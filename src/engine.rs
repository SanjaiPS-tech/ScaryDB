...
pub struct DatabaseState {
    ...
    pub data: FastMap<u32, FastMap<u32, Value>>, // BUCKET_ID -> KEY_ID -> Value
}

pub struct StorageEngine {
    pub global_catalog: Catalog,
    pub databases: FastMap<u32, DatabaseState>,
}
...
impl StorageEngine {
    ...
    pub fn create_bucket(&mut self, db_id: u32, bucket_name: &str) -> Result<u32, String> {
        let db_state = self.databases.get_mut(&db_id)
            .ok_or_else(|| format!("Database with ID {} not found in storage", db_id))?;
        ...
        db_state.data.insert(bucket_id, FastMap::default());
        Ok(bucket_id)
    }
    ...
    pub fn drop_bucket(&mut self, db_id: u32, bucket_name: &str) -> Result<u32, String> {
        ...
        let bucket_id = db_state.db_catalog.get_bucket_id(bucket_name)
            .ok_or_else(|| format!("Bucket '{}' does not exist", bucket_name))?;
    ...
    pub fn set_key(&mut self, db_id: u32, bucket_name: &str, key_name: &str, value: Value) -> Result<(), String> {
        ...
        let bucket_id = db_state.db_catalog.get_bucket_id(bucket_name)
            .ok_or_else(|| format!("Bucket '{}' does not exist. Create it first.", bucket_name))?;

        let bucket_data = db_state.data.get_mut(&bucket_id)
            .ok_or_else(|| format!("Bucket data storage for ID {} not initialized", bucket_id))?;

        let key_catalog = db_state.db_catalog.bucket_key_catalogs.get_mut(&bucket_id)
            .ok_or_else(|| format!("Key catalog for Bucket ID {} not initialized", bucket_id))?;
...
// Custom extension trait to help with Result conversions on Option
trait OptionExt<T> {
